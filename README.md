...
You will notice that `150` (our result) is left at the base of the stack at `M[256]`. This is also the address of the `LCL` and `ARG` registers before we made the jump(s) to `Math.mult`. By design, the return argument will be at `M[ARG]` when a function call returns. Additionally, `SP` will be equal to `ARG + 1` (unless we support multiple return arguments and we don't). If the caller (to which we return) has no locals then `LCL` will be equal to `ARG` and both will point to the base of the current stack frame.
...
* Terence Parr for **ANTLR** and **StringTemplate** which now have become indispensable in most of my leisure programming. Having these tools was a huge time saver when implementing the various parsers and transpilers for this project.
